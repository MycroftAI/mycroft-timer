...
# Design update
  Scenario Outline: status of more than 3 timers
    Given an english speaking user
      And no timers are previously set
      And a 5 minute timer is set
      And a 1 hour timer is set
      And a 20 minute timer is set
      And a 45 minute timer is set
      When the user says "<what's the status of the timers>"
      Then "mycroft-timer" should reply with "There are four timers running, timer 1 for 10 minutes with 8 minutes remaining, timer 2 for 1 hour, timer 3 for 20 minutes, and timer 4 for 45 minutes, would you like the remaining time on another timer?"
